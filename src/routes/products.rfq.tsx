...
export const Route = createFileRoute("/products/rfq")({
  ...
  component: () => (
    <ProductPage
      ...
      rightPanel={<RFQPanel />}
      features={[
        { icon: <FileText className="h-5 w-5" />, t: "RFQ builder", d: "Templates and structured forms for repeatable sourcing events." },
        { icon: <Send className="h-5 w-5" />, t: "Supplier invitations", d: "Invite suppliers with a single click and track responses." },
        { icon: <GitCompare className="h-5 w-5" />, t: "Quote comparison", d: "Side-by-side comparison of price, delivery and terms." },
        { icon: <ScrollText className="h-5 w-5" />, t: "Version history", d: "Full audit trail of every revision and clarification." },
        { icon: <CheckCircle2 className="h-5 w-5" />, t: "Approval workflow", d: "Route awards through finance, legal and procurement." },
        { icon: <Award className="h-5 w-5" />, t: "Award supplier", d: "Award, split-award and notify suppliers in one click." },
      ]}
      benefits={[
        "Cut RFQ cycle time by up to 50%",
...
function RFQPanel() {
  ...
  return (
    <div className="surface-card p-5 max-w-md ml-auto">
      <div className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Quotation Comparison</div>
      <table className="mt-4 w-full text-xs">
        ...
        <tbody>
          {rows.map((r, i) => (
            ...
            >
              <td className="py-2.5 text-primary font-medium">{r.s} {r.best && <span className="ml-1 text-[9px] font-bold text-emerald-700">✓</span>}</td>
