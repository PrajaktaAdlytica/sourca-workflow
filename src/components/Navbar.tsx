  ...
  return (
    ...
    >
      <nav className="mx-auto flex h-16 max-w-7xl items-center justify-between px-6">
            ...
            {productsOpen && (
              <div className="absolute left-0 top-full pt-2 w-[320px]">
                <div className="surface-card p-2 shadow-xl">
                  {products.map((p) => (
                    <Link key={p.to} to={p.to} className="block rounded-lg px-3 py-2.5 hover:bg-secondary transition-colors">
        ...
        <button className="lg:hidden p-2 -mr-2 text-primary" onClick={() => setOpen(!open)} aria-label="Toggle menu">
      ...
      {open && (
        <div className="lg:hidden border-t border-border bg-background/95 backdrop-blur">
          <div className="px-6 py-4 space-y-1">
            ...
            {products.map((p) => (
              <MobileLink key={p.to} to={p.to} onClick={() => setOpen(false)}>{p.name}</MobileLink>
            ))}
...
function NavLink({ to, children }: { to: string; children: React.ReactNode }) {
  return (
    <Link to={to} className="px-3 py-2 rounded-md text-primary/80 hover:text-primary transition-colors"
...
function NavAnchor({ href, children }: { href: string; children: React.ReactNode }) {
  return <a href={href} className="px-3 py-2 rounded-md text-primary/80 hover:text-primary transition-colors">{children}</a>;
}
function MobileLink({ to, children, onClick }: { to: string; children: React.ReactNode; onClick: () => void }) {
  return <Link to={to} onClick={onClick} className="block py-2.5 text-primary">{children}</Link>;
}
