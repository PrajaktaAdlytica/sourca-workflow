    ...
    <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-4">
      ...
      >
        <table className="mt-4 w-full text-[10px]">
          ...
          <tbody className="text-primary/80">
            {[["A", "$4.28", "12d"], ["B", "$4.65", "15d"], ["C", "$4.90", "18d"]].map((r) => (
              <tr key={r[0]} className="border-t border-border">
                <td className="py-1">Supplier {r[0]}</td><td className="py-1 tabular-nums">{r[1]}</td><td className="py-1 tabular-nums">{r[2]}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      >
        <div className="mt-4 relative h-16">
          ...
          <div className="absolute right-0 top-1/2 -translate-y-1/2 h-8 w-8 rounded-md bg-accent text-accent-foreground flex items-center justify-center text-[9px] font-bold">S</div>
        </div>
      </Card>
    </div>
  );
...
function Card({ icon, title, desc, children }: { icon: React.ReactNode; title: string; desc: string; children?: React.ReactNode }) {
  return (
    <div className="surface-card card-hover p-6 flex flex-col">
      <div className="h-10 w-10 rounded-lg bg-accent-soft text-accent flex items-center justify-center">{icon}</div>
