import { LayoutDashboard, Building2, FileText, ScrollText, Shield, FileCheck, BarChart3, Settings, TrendingUp } from "lucide-react";
      ...
      <div className="flex min-h-[560px]">
        ...
        <div className="w-[200px] shrink-0 bg-primary text-primary-foreground p-4 hidden md:block">
          ...
          <div className="mt-6 space-y-1">
            {navItems.map((n) => (
              <div key={n.label} className={`flex items-center gap-2.5 px-3 py-2 rounded-md text-xs ${n.active ? "bg-accent text-accent-foreground font-medium" : "text-primary-foreground/70 hover:text-primary-foreground"}`}>
        ...
        <div className="flex-1 p-5 space-y-4 bg-surface">
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="text-[11px] text-muted-foreground">May 1 – May 31, 2026</div>
            </div>
            <button className="h-8 px-3 rounded-md border border-border text-[11px] text-primary/80 hover:border-primary/30">Export</button>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
            ...
            <div className="lg:col-span-2 surface-card p-3.5">
              <div className="text-[10px] text-muted-foreground uppercase tracking-wider">Recommended Supplier</div>
              <div className="mt-2.5 flex items-center gap-2.5">
                <div className="h-8 w-8 rounded-md bg-accent text-accent-foreground flex items-center justify-center text-[10px] font-bold">EC</div>
                ...
                <span className="text-[9px] font-medium px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-700 ring-1 ring-emerald-200">Best Match</span>
                <div className="text-[11px] text-primary">Score <span className="font-semibold tabular-nums">92</span></div>
                <button className="text-[10px] text-accent font-medium hover:underline">View Details</button>
              </div>
            </div>
          </div>
        </div>
...
function ScoreCircle({ label, value, verdict, color, small }: { label: string; value: number; verdict: string; color: "emerald" | "amber"; small?: boolean }) {
      ...
      <div className="mt-2 flex items-center gap-3">
        <div className="relative h-16 w-16">
          <svg viewBox="0 0 64 64" className="h-16 w-16 -rotate-90">
            ...
            <motion.circle
              cx="32" cy="32" r={r} className={stroke} strokeWidth="6" fill="none" strokeLinecap="round"
          ...
          <div className="absolute inset-0 flex items-center justify-center text-sm font-semibold text-primary tabular-nums">{value}</div>
        </div>
